...
class MockTimers {
  ...
  #createDate() {
    ...
    ObjectDefineProperties(
      MockDate,
      // Disabling next rule because this is exactly what we want to do
      // eslint-disable-next-line node-core/avoid-prototype-pollution
      ObjectGetOwnPropertyDescriptors(this.#realDate),
    );
    ...
    MockDate.toString = function toString() {
      return MockDate[kMock].#realDate.toString();
    };
  ...
  #isValidDateWithGetTime(maybeDate) {
    ...
    const getTime = this.#realDate.prototype.getTime;
    try {
      getTime.call(maybeDate);
      return true;
    } catch {
      return false;
    }
  ...
  enable(...args) {
    ...
    if (args.length <= 0) {
      this.#timersInContext = SUPPORTED_TIMERS;
      this.#now = 0;
      this.#toggleEnableTimers(true);
      return;
    }

    let timersDefined = false;
    // Checks if the first argument is the timer array or the initial time
    if (ArrayIsArray(args[0])) {
      validateArray(args[0], 'timers');
      // Check that the timers passed are supported
      ArrayPrototypeForEach(args[0], (timer) => {
      ...
      this.#timersInContext = SUPPORTED_TIMERS;
    } else if (validateNumber(args[0], 'initialTime') === undefined) {
      // First argument is the initial time as number
      this.#assertTimeArg(args[0]);
      this.#now = args[0];
      this.#timersInContext = SUPPORTED_TIMERS;
    } else {
      throw ERR_INVALID_ARG_VALUE(
        'enableTimers',
        args[0],
        'the first argument must be an array of strings, a Date instance or a positive integer',
      );
    }

    // Checks if the second argument is the initial time
    if (args[1] && timersDefined) {
      if (this.#isValidDateWithGetTime(args[1])) {
        this.#now = args[1].getTime();
      } else if (validateNumber(args[1], 'initialTime') === undefined) {
        this.#assertTimeArg(args[1]);
        this.#now = args[1];
      } else {
        throw ERR_INVALID_ARG_VALUE(
          'initialEpoch',
          args[1],
          'initial epoch must be a Date instance or a positive integer',
        );
      }
    }
  ...
  /**
   * @param {number | Date} time The epoch to set the current time to.
   */
  setTime(time = 0) {
    let assertedTime = time;
    if (this.#isValidDateWithGetTime(time)) {
      assertedTime = time.getTime();
    } else {
      validateNumber(time, 'time');
      this.#assertTimeArg(assertedTime);
      this.#assertTimersAreEnabled();
    }

    if (assertedTime < this.#now) {
      this.#now = assertedTime;
      return;
    }

    this.tick(MathAbs(assertedTime - this.#now));
  }
