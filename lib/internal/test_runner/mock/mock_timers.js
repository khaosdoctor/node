...
/**
 * @typedef {{timersToEnable: string[];now: number | Date;}} EnableOptions Options to enable the timers
 * @property {string[]} timersToEnable List of timers to enable, defaults to all, supported values are 'setTimeout', 'setInterval', 'Date'
...
class MockTimers {
   ...
   */
  enable(options = { timersToEnable: SUPPORTED_TIMERS, now: 0 }) {
    if (this.#isEnabled) {
      throw new ERR_INVALID_STATE('MockTimers is already enabled!');
    }

    validateArray(options.timersToEnable, 'timers');

    // Check that the timers passed are supported
    ArrayPrototypeForEach(options.timersToEnable, (timer) => {
    ...
    this.#timersInContext = options.timersToEnable;

    // Checks if the second argument is the initial time
    if (this.#isValidDateWithGetTime(options.now)) {
      this.#now = DatePrototypeGetTime(options.now);
    } else if (validateNumber(options.now, 'initialTime') === undefined) {
      this.#assertTimeArg(options.now);
  ...
  runAll() {
    ...
    // FIXME: this is open to discussion, should we return to the original time
    // or should we calculate the last timer to run and add that to the
    // oritinal time?
