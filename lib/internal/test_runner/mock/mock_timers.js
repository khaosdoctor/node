...
class MockTimers {
   ...
   */
  enable(options = { apis: SUPPORTED_TIMERS, now: 0 }) {
    if (this.#isEnabled) {
    ...
    if (!options.apis) {
      options.apis = SUPPORTED_TIMERS;
    }

    validateArray(options.apis, 'timers');
    // Check that the timers passed are supported
    ArrayPrototypeForEach(options.apis, (timer) => {
    ...
    this.#timersInContext = options.apis;
