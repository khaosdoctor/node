...
let kResistStopPropagation, kMock;
...
class MockTimers {
  ...
  #createDate() {
    ...
    for (const prop in this.#realDate) {
      if (ObjectPrototypeHasOwnProperty(this.#realDate, prop)) {
  ...
  #isValidDateWithGetTime(maybeDate) {
    return (
      maybeDate instanceof globalThis.Date &&
      'getTime' in maybeDate &&
      typeof maybeDate.getTime === 'function'
    );
  }
